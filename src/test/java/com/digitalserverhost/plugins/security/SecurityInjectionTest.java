...
public class SecurityInjectionTest {
    ...
    void testSqlInjectionInUsername() throws SQLException {
        // Malicious username attempting to break out of single quotes or comment out the rest of the query
        String maliciousName = "Notch'; DROP TABLE player_data; --";
        UUID uuid = UUID.randomUUID();

        // Testing updateLastKnownName
        databaseManager.updateLastKnownName(uuid, maliciousName);

        // Verify that the malicious string was passed as a SINGLE parameter, not part of the SQL
    ...
    void testSqlInjectionInTableName() {
        // Table name is escaped in constructor: this.tableName = "`" + tableName.replace("`", "") + "`";
        ...
        assert(!escaped.contains("``")); // The backtick in the middle should have been replaced
    }

    @Test
    void testSqlInjectionInUuid() throws SQLException {
        String maliciousUuid = "00000000-0000-0000-0000-000000000000' OR '1'='1";
        // Note: UUID.fromString would fail here, but if we pass it as a string to DB manager
        // we want to ensure it's parameterized.
        
        // Let's assume we have a method that takes a UUID string or we mock the UUID object
